...
procedure TFrmStockOut.Button1Click(Sender: TObject);
var
  SQLCmd: String;
begin
  SQLCmd := '';
  ...
  begin
    Active := False;
    SQL.Text := 'select * from TranH where Type_=1';
    SQL.Add(SQLCmd);
...
procedure TFrmStockOut.Button2Click(Sender: TObject);
begin
  Close;
end;

procedure TFrmStockOut.Button3Click(Sender: TObject);
var
  WH: TStock;
begin
  with cdsTranB do
...
procedure TFrmStockOut.cdsTranBFinal_GetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TFrmStockOut.cdsTranBNewRecord(DataSet: TDataSet);
begin
  with DataSet do
  begin
...
procedure TFrmStockOut.cdsTranBPartCode_Change(Sender: TField);
var
  cdsTmp: TADOQuery;
begin
...
procedure TFrmStockOut.cdsTranBPartCode_Validate(Sender: TField);
var
  cdsTmp: TADOQuery;
begin
  cdsTmp := TADOQuery.Create(Self);
...
procedure TFrmStockOut.cdsTranHNewRecord(DataSet: TDataSet);
var
  sGUID: string;
begin
  sGUID := CreateClassID();
  with DataSet do
  begin
    FieldByName('Type_').AsInteger := 1; //0.进货单; 1.出货单; 2.盘点单
    FieldByName('TBNo_').AsString := GetNewTBNo('OU');
...
procedure TFrmStockOut.cdsTranHType_GetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TFrmStockOut.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmStockOut.FormCreate(Sender: TObject);
begin
  PageControl1.ActivePageIndex := 0;
end;

function TFrmStockOut.GetNewTBNo(const TB: String): String;
var
  i: Integer;
  YM: String;
  No: String;
...
procedure TFrmStockOut.TabSheet2Show(Sender: TObject);
begin
  with cdsTranH do
  begin
    if not Active then
